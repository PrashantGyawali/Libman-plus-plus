...
useEffect(()=>finalfiltered(unfilteredbooks),[tags,authorname,bookname,unfilteredbooks]); //whenever the options in filter option div changes re render the list
    ...
    return (
                <div>
                    ...
                    <div className="container-fluid justify-content-between align-items-baseline px-2 optionsdiv">
                        <div className='d-flex align-items-baseline'>
                            <div className={`px-2  ${draftorbook==='allbooks'? "h1" : "h3 text-muted mx-2"}`} onClick={()=>{setdraftorbook('allbooks')}} style={{cursor:'pointer'}}>Books</div>
                            <div className={`px-2  ${draftorbook==='alldrafts'? "h1 " : "h3 text-muted mx-2"}`} onClick={()=>{setdraftorbook('alldrafts')}} style={{cursor:'pointer'}}>Drafts</div>
                        </div>
                    ...
                    <div className='row m-0 align-items-baseline' style={{justifyContent:'flex-start', padding:'0px 1px'}}>
                        {filteredBooks && filteredBooks.map((e)=><BookComponent data={e} key={e.id} togglenewbookadding={togglenewbookadding}/>)}
                    </div>

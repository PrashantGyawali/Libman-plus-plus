...
function Booklistdiv(props) {
  ...
  useEffect(() => {
    props.newbookadding && setfilteredoptiondiv(false);
  }, [props.newbookadding]);
        ...
        const areEqual = tags
          ? tags.every(
              (obj) =>
                ...
                e.tags.some(
                  (otherObj) => JSON.stringify(obj) === JSON.stringify(otherObj)
                )
            )
          : true;

        return (
          (bookname ? e.bookname.match(re) : true) &&
          (authorname ? e.authors.match(re2) : true) &&
          areEqual
        );
      });
  ...
  const refreshfilter = () => {
    //refresh the filter i.e show all books once again
    setauthorname("");
  ...
  useEffect(
    ...
    [tags, authorname, bookname, unfilteredbooks]
  ); //whenever the options in filter option div changes re render the list

  return (
    <div>
      <div
        style={{ borderBottom: `3px solid  ${dark ? "white" : "grey"}` }}
      ...
      >
        <div className="container-fluid justify-content-between align-items-baseline px-2 optionsdiv background-white">
          <div className="d-flex align-items-baseline background-white">
            <div
              className={`px-2  ${
                draftorbook === "allbooks" ? "h1 txt-dark" : "h3 txt-muted mx-2"
              }`}
              onClick={() => {
                setdraftorbook("allbooks");
              }}
            ...
            <div
              className={`px-2  ${
                draftorbook === "alldrafts"
                  ? "h1 txt-dark"
                  : "h3 txt-muted mx-2"
              }`}
              onClick={() => {
                setdraftorbook("alldrafts");
              }}
              style={{ cursor: "pointer" }}
            >
              Drafts
          ...
          <div style={{ display: "inline" }} className="background-white">
            <div
              className="btn btn-sec py-0 m-1"
              style={{ fontSize: 20 }}
              onClick={() => {
                !props.newbookadding && setfilteredoptiondiv(!filteroptiondiv);
              }}
            >
              Tools
            </div>
            <button
              className="btn btn-sec py-0 "
              style={{ fontSize: 20 }}
      ...
      <AnimatePresence>
        {filteroptiondiv && (
          <Filteroptions {...{ settags, setBookname, setauthorname }} />
        )}
      </AnimatePresence>

      <div
        className="row m-0 align-items-baseline background-light"
        style={{ justifyContent: "flex-start", padding: "0px 1px" }}
      >
        {filteredBooks.length > 0 ? (
          ...
          <div className="container">
            <h3 className="txt-dark m-3" style={{ fontStyle: "italic" }}>
              "A bare bookshelf is not a sign of a lack of knowledge, but rather
              an invitation to explore new worlds and fill it with the stories
              that move you."
            </h3>
          </div>
        )}
